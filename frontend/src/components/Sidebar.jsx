...
export function Sidebar(){
  const [fullSidebar, setFullSidebar] = useState(true)
   return <div className={`max-sm:hidden min-h-screen ${ fullSidebar ? 'w-56' : 'w-20'} bg-slate-100 flex flex-col justify-between dark:bg-neutral-800 `} id="sidebar" >
    <div>
     ...
     <>
      <div className="flex justify-center items-center mt-10 bg-white dark:bg-neutral-800 shadow-lg rounded-full h-10 w-10 ml-4 cursor-pointer">
        <img className={"h-8 w-6 dark:invert"} src={assets.plus_icon} alt="" />
      </div></>}

     {fullSidebar && <div className="ml-5 mt-10 font-semibold dark:text-white">Recent</div> } 
      </div>
      <div className="flex flex-col gap-6 ml-5 mb-10">
       ...
       {fullSidebar && <div className="col-span-2 dark:text-white">Help</div> } 
        </div>
        ...
        {fullSidebar && <div className="col-span-2 dark:text-white">Activity</div> } 
        </div>
        ...
        {fullSidebar && <div className="col-span-2 dark:text-white">Settings</div> } 
        </div>
      </div>
   </div>
}